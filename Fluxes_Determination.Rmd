...
As can be seen, the IRGA in some plots adds many points that rise or fall quickly toward the end. Therefore, we will filter up to the measurement time defined when setting up the respiration chambers (in this case, 180).

Since the measurement time is not included in the data provided by the IRGA, we created a new variable with the difference in the record number (which we can use for filtering the plot time)
